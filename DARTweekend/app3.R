...
server <- function(input, output) {
  table <- renderDataTable({
    ...
    query <- paste0("SELECT ArrivalTime, RouteID, RedStation FROM WeekendArrivalTimes WHERE RouteID LIKE '%", input$route, "%' AND RedStation LIKE '%", input$station, "%'")
    table_df = dbGetQuery(conn, paste0(query, " LIMIT ", input$nrows, ";"))
  }, escape = FALSE)
  
  output$tbl <- table
  
  table2 <- renderDataTable({
    
    # sqllite
    # Be sure the data file must be in same folder
    sqlite_conc <- dbConnect(RSQLite::SQLite(), dbname ='Dallas Area Rapid Transit.db')
    conc=sqlite_conc
    
    on.exit(dbDisconnect(conc), add = TRUE)
    query2 <- paste0("SELECT * FROM RedStations WHERE RedStation LIKE '%", input$station, "%'")
    table_df = dbGetQuery(conc, paste0(query2, ";"))
  }, escape = FALSE)
